...
init がフック後に走るよう launch.py をサスペンド起動化済み（`boot/README.md`）。

---

## 現状: native amBackup を使用（stub 撤去済み 2026-06-13）

以前は `boot/ambackup/stub.js` が `amBackupReadRecord`(0x9858D0) を `MOV EAX,-21; RET 8` に固定し、
amBackup を全死させていた（crash 回避の暫定）。mxsram を micetools 準拠＋永続化したことで native パスが
実データで成立するため、**stub を撤去し native amBackup を再活性化**。詳細は
`docs/bugs_archive.md` [FIXED] amBackupWrite access violation。
...
| 0x9858D0 | amBackupReadRecord | ※旧名 amBackupWrite_entry（誤named）。`*param_1`=area。0x9858F3 が旧 crash 点 |
| 0x9858E1 | amBackupReadRecord_body | 旧 stub の jne target |
