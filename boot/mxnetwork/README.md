...
現状は ALL.Net 接続段を **satisfy（resolved 固定）/ billing offline** で通すだけ（`./state.js` 0x72DCE0 connection / `../ambilling/status.js` 0xA065C0 billing）。
...
- **方針**: `network_role=serve` の層（`mxnetwork/*`, `mxkeychip/client`, `ambilling/status.js`(0xA065C0), `mxnetwork/state.js`(0x72DCE0)）は
