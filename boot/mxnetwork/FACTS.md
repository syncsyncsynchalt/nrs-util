...
`result` を見つけられず -1 → SM ループ → Error 8006 になる。`boot/mxkeychip/server/pcpa_server.py` は
`&` 区切りで応答し、native 成功（ret=0, dhcp_status=3, nic_ready=1）する。`mxnetwork/diag.js` は log-only monitor。
