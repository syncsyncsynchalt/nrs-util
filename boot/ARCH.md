...
サブシステム別の事実は `boot/<subsys>/FACTS.md` に置く（索引は repo ルート `FACTS.md`）。
...
- Ghidra MCP / `tools/static/disasm.py` / boot シム / モジュールヘッダ `// va:` / `known_names.json` キー /
  FACTS / BUGS は全て static_VA。
- 生 `nrsBase.add(...)` は `tools/hygiene/check_doc_sync.py` が禁止する（va() 経由を強制）。
