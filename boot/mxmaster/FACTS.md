...
`boot/mxkeychip/server/pcpa_server.py` がサーバ側で応答する（micetools `micemaster/callbacks/foreground.c` 相当）。

応答仕様（[F]）: `mxmaster.foreground.getcount=N` → `getcount=N` ＋別行 `count=0`（`code=0` のみだとゲームが
~2 秒で終了）。`foreground.active=?`/`current=?` → `=1`。詳細は `../mxkeychip/FACTS.md` の PCPA 節と pcpa_server。
