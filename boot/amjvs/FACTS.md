...
### bypassJvs IIFE (`boot/amjvs/state.js`) — patchCode + 初期 state write（全 Interceptor 廃止）

commit 56cb7b2 で amJvspInit/StatusCheck/NodeInfo の Interceptor 3 本を撤去。現行は patchCode 永続 + ロード時の state 直接書き込みのみ。
...
参考（**現在は強制していない** — 旧 amJvspNodeInfo hook が書いていた値。FUN_0067afa0 patchCode で nodeInfo version-check 経路に到達しないため不要）:
node_info_buf base から +0x101=0x14 / +0x108=0x02 / +0x124=0x0d / +0x134=0x13 / +0x138=0x00
...
| 種別 | RVA | Function | 内容 |
|---|---|---|---|
| patchCode | 0x5883D3 | amJvspAckSwInput 内 | `MOV EAX,EDI`(8B C7) → `XOR EAX,EAX`(33 C0)。GetReport(-11) failure path を 0 返しに（commit Tier3、永続）。これにより下の Interceptor は -11 を見ずに済む |
| Interceptor | 0x588360 | amJvspAckSwInput(report_buf, node_0x306, panel_idx, count, out_switch_ptr) | onEnter で args[4] 保存、onLeave で switch bytes を phase 別に書込 |
| Interceptor | 0x5884F0 | amJvspGetCoinCount(report_buf, node_0x306, chute_1idx, out_coin) | onLeave: coin count inject |
| Frida timer | setInterval(2s) | — | JVS watchdog: initFlag/nodeCount/poll_state を毎 tick 強制（**残存 Frida 依存**） |

Injection phase sequence (onEnter で args[4] を保存、onLeave で書込):
- Phase 0: wait 20 P1 calls (~40s at init rate ~0.5Hz)
- Phase 1: SERVICE (5 frames, args[4].writeU8(0x40))
- Phase 2: wait 15 calls
- Phase 3: START (5 frames, args[4].writeU8(0x80))
- Phase 4: done
...
Shared memory: `TeknoParrot_JvsState` (8 bytes) — **生成者=`TeknoParrot.dll`(32bit) を実行時観測で確定** [F]
（2026-06-11, `jvsstate_capture.py` attach。caller=`TeknoParrot.dll+0x4265` 他、最深=VMProtect entry `+0x736b778`）。
