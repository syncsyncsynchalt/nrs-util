...
`boot/keychip/server/pcpa_server.py` を `&` 区切りに直して native 成功（ret=0, dhcp_status=3, nic_ready=1）を
実機実証。旧 `0x5814E0` force-patch は撤去（`amnet/state.js` は log-only monitor 化）。詳細 `BUGS.md`。
