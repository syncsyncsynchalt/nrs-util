...
**未エミュ時の症状**（`devices.js classify()` に columba が無いと）:
`\??\columba` open が `0xc0000034`(NOT_FOUND) → ① `FUN_00988c10` が成功時しか `DAT_01296158` を
キャッシュしないため**毎回 columba を再 open（洪水）** ② board type 不明 → `getAreaDescriptor` が null
→ `amBackupWrite` が `-21`(0xffffffeb) → `amBackupRecordWriteDup: error(-21)` 連発。
1 WriteDup = `amBackupWrite` ×2(dup) = columba ×2 → ログは columba open ≈ 2×error(-21)。
