...
最後の 0903 を TeknoParrot 方式（チェック無力化＝`mxkeychip/region.js`）で恒久解消したのが決め手。残る理論リスクは
...
実機 latch 観測=0xa→0x4→0x14。**handled errCode の fix は `boot/patches.json`**、各 setter 関数は要時 Ghidra
`search_decompiled` で再導出する（正は実装）。**未対応 errCode（要時 setter を NOP）**: 0xd=`FUN_0045a320` /
0xe=`FUN_004591b0` / 0x18=`FUN_0050a340` / 0xa board-table=`FUN_00679cb0`(10 は未抑止の疑い)。
...
**方針統一（2026-06-13）**: TeknoParrot 方式＝**チェック無力化(A)に統一**する（TP は dongle/mxkeychip/region
...
最初に (A) へ変換済。残りの latch コード（0xa board-table / 0x14 network 等）も
