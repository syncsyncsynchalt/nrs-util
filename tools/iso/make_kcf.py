...
SEED=binascii.unhexlify("DB86373A5A2E05B963C282D789128D0D")  # usable な appboot seed
KEY =binascii.unhexlify("6ACB8DC90049927AEACF71C9740B6FF9")  # usable な AES key
IV  =binascii.unhexlify("A47A668EC0DA675E10E3A3EBE5328CF0")  # usable な AES iv

def swap(b,a1,a2,a3,a4):  # 自己逆: file = swap(usable) なので MiceParseKcf の swap(file)=usable
...
struct.pack_into('<I',hdr,4,1)            # m_Format: byte (micekeychip は %02X で送る); KeychipCheck
                                          # は formattype==1 を要求。<=0xFF 必須 (3バイトバッファ)
...
struct.pack_into('<I',hdr,0x14,0x0035A8C0) # m_NetworkAddr 192.168.53.0 (HTML のサブネット由来)

kcf=bytearray()
kcf+=hdr
kcf+=bytes(216)        # m_AppData
kcf+=seed_f            # m_Seed (scramble 済)
kcf+=key_f             # m_Key (scramble 済)
kcf+=iv_f             # m_Iv (scramble 済)
kcf+=bytes(128)        # Unk
