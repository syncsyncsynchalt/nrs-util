...
> ⚠️ TP は `192.168.168.0/24`。本プロジェクトの amNet bind 先は `192.168.1.209`（`../boot/amnet/FACTS.md`）で
...
|---|---|---|
| region | プロファイル `DongleRegion`/`PcbRegion=JAPAN` を注入時に供給 | `pcpa_server.py` で `keychip.appboot.region`=01、`boot/keychip/region.js` で setter 無力化（Error 0903） |
| keychip | TP が keychip 応答をエミュレート | `pcpa_server.py` + keychipSM 完走（`../boot/keychip/FACTS.md`） |
| amNet | プロファイル IP(192.168.168.x) で応答 | bind 先を INADDR_ANY 化、`pcpa_server.py` の `&` 区切り応答（`../boot/amnet/FACTS.md`） |
| ドングル | TP がドングル検査を通過 | `boot/amdongle/monitor.js` / `patch.js`（amDongleBusy のみ維持） |
...
| amNet 応答・bind 先供給 | `boot/amnet/state.js` + `pcpa_server.py` |
| ドングル/amPlatform/region 検査通過 | `boot/amdongle/*` / `boot/amplatform/identity.js` / `boot/keychip/region.js` |
| amGfetcher / get_status | `boot/amgfetcher/getstatus.js` |
| JVS バイパス・デバイス擬装 | `boot/amjvs/*` / `boot/mxdrivers/devices.js` |
| keychip/PCPA・billing offline | `pcpa_server.py` / `boot/keychip/setup.js` / `boot/allnet/billing.js` |
| IC Card/USB I/O/Touch/Network type/EXTEND/ALL.Net/P-ras | `boot/devices/*` / `boot/amnet/network_type.js` / `boot/startup/*` / `boot/allnet/connection.js` |
