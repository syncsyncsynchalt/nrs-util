...
> ⚠️ TP は `192.168.168.0/24`。本プロジェクトの amNet query_nic_status 応答 IP は `192.168.1.209`（`../boot/mxnetwork/FACTS.md`）で
> **サブネットが異なる**。TP 値をそのまま流用せず amNet の応答 IP を正とする。
...
| amNet | プロファイル IP(192.168.168.x) で応答 | `pcpa_server.py` の `&` 区切り応答（bind はシム未介入＝フック撤去済み。`../boot/mxnetwork/FACTS.md`） |
...
| amNet 応答（query_nic_status の NIC IP 等。bind はシム未介入） | `pcpa_server.py` 応答 + `boot/mxnetwork/diag.js`（観測のみ） |
...
| keychip/PCPA・billing offline | `pcpa_server.py` / `boot/mxkeychip/setup.js` / `boot/ambilling/status.js`(0xA065C0) |
| IC Card/USB I/O/Touch/Network type/EXTEND/ALL.Net/P-ras | `boot/devices/*` / `boot/mxnetwork/state.js`(0x6FF1B3) / `boot/mxsegaboot/*` / `boot/mxnetwork/state.js`(0x72DCE0) |
