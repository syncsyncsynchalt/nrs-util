# RingEdge System Software v63.01.10 要約（SEGA 純正システムイメージ）
...
- 関連: `docs/micetools.md`（am*/keychip クリーンルーム実装）、`docs/bsnk_ringedge.md`（PCP/DS28CN01 散文）、
  `docs/teknoparrot.md`（上位レイヤ）、`../boot/mxkeychip/FACTS.md`、`../boot/mxkeychip/server/pcpa_server.py`。
...
SSD proof / RSA、`boot/mxkeychip/setup.js`）の裏取りに使う。**全文は実ファイルを直読**。
...
→ 解析経路は **(a) 静的 Ghidra 取り込みが第一選択**。実行時観測(b)は CrackProof 下でのデバッガアタッチが要る場合の
  補助。確認済みの PCP キー全集合・内部ハンドラ名・暗号方式は `boot/mxkeychip/FACTS.md`（裏取り済み）。

判明した実装事実（`boot/mxkeychip/FACTS.md` に反映済）:
