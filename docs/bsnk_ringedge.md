# bsnk RingEdge ドキュメント（SEGA RingEdge 解析の散文解説）
...
- `pcpaInitStream()` / `pcpaClose()`：ストリーム確保と終了
...
6. **ゲーム⇄keychip ハンドシェイク（AmLib）**：`amDongleSetAuthConfig()` で追加認証を有効化する。
...
## 4. ALL.Net ネットワーク（`/allnet/`。世代共通で参照可）
...
## 関連
- `docs/micetools.md`：同一作者の **C 実装**（このサイトの裏付けと正準値）
