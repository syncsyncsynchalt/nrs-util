...
- **Frida は解析の足場に降格する**（Ghidra と並ぶデバッグ専用ツール）。出荷物に Frida を含めない。
- 構成は現行の `boot/<subsys>/` と `boot/MANIFEST.json` と `cabinet/*.toml` をそのまま土台にする。
...
→ satisfy を決定的 `patchCode`/served に寄せればレースが消える（原理1と2）。

## ロードマップ（優先順）

- **P0 安定アトラクトの回復**（足場）。再ブートループと device-satisfy レースを安定化。クリーン環境で
  `boot/launch.py --spawn` が確実にアトラクト到達することを実写確認できる状態にする。
- **P1 実行時 Frida 依存の根絶**（差分①②）。MANIFEST の `persistence=runtime` 群を撤廃する。
...
- **P2 keychip/PCPA サーバを micetools 級に**（差分④⑤）。`pcpa_server.py` を keyword 別
  `callbacks/<group>.py` に分解する。暗号は当面 `code=54` bypass で足りる。network/billing を本格化するなら
  KCF/AES を実装する（micetools が正準）。
- **P3 ネットワーク層は追加で**（差分⑥）。`boot/mxnetwork/` の予約枠に PowerOn/DownloadOrder/
  Billing/AiMeDB/タイトルサーバを足す。SP=loopback / MP=共有ホストの切替は `cabinet` の
  `network_role=serve` 設計で種を撒いてある。今やる必要はないが seam は維持する。
...
これでロード順の脆さと device-satisfy レースと版差脆弱性が消え、TP の堅牢さと micetools の正準性に近づく。
