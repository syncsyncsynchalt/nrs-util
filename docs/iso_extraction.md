...
抽出の観点では、データが平文になるのは TrueCrypt マウント時点なので、本当に要るのは original0 コンテナと正しい鍵ファイルである（詳細は後述の抽出の章）。
...
このフェーズで mxsegaboot が決めるのは、ゲームを SSD へインストールし直す必要があるかどうかである。
判断材料は二つ、SSD スロットの状態と、ネットワークからの配信指示だ。

純正ではまず SlotCheckWait（FUN_00412a50）が mxinstaller / mxstorage にスロットの状態を問い合わせる。
返るのは complete / empty / error などで、エミュレーション（micetools 上）では `query_slot_status` の応答として実際に観測した（original0 = complete、patch0 = empty）。
続く InstallJudg（FUN_00413df0）が、このスロット状態とネットワーク側の配信状況を突き合わせて要否を決める。
スロットが空か配信指示が来ていればインストールへ、すでに complete で配信もなければそのまま起動へ進む。

判定がネットワークに依存すること自体は確認できている。
ただしその配信が何を意味するか、すなわち正規環境では mxgcatcher（BeaconCatcher）が ALL.Net の配信指示（download order / beacon）を受け取って初めて「インストール必要」が立つ、という解釈のほうは、bsnk の資料と、文字列や挙動からの推定にとどまる。

検証での狙いは、DVD からの展開経路（フェーズ C）を観測することにある。
そのためにはスロットが complete でもインストール側へ倒したいが、オフライン環境では配信指示が来ないので、放っておくと InstallJudg はインストール側へ進まない。
ただし判定そのものを飛ばすわけにはいかない。
InstallJudg 内部の REMOUNT_DEVICE が抽出対象の ISO を Q: にマウントする副作用を持ち、これが後段で要るからだ。
そこで通信応答を偽装するのではなく、InstallJudg は走らせたまま、ExecServer がその結果を見て分岐する箇所だけを上書きし、常に「インストール必要」側へ倒した。
この上書きで、次のフェーズ C（DvdDiscWait → AlphaDvdInstall）へ進む。

もう一箇所、SlotCheckWait から InstallJudg への前進にもパッチを当てている。
SlotCheckWait は正常終了するが void 返りのため、残ったレジスタ値を ExecServer が失敗と誤認し、エラー状態へ逸れてしまう。
この分岐を補正して、正しく InstallJudg へ進めた。
当てた番地は付録に記す。
...
|---|---|---|---|
| 媒体と中身 | install前のキーチップ暗号データ（sector 2112〜） | install済み TrueCrypt コンテナ | install済み TrueCrypt コンテナ |
| TrueCryptコンテナ(original0) | ❌ 無い | ✅ ある | ✅ ある |
| ATA解錠 | — | 静的 ATA キーで解錠（暗号化ではない） | 同左 |
| 鍵ファイルの出所 | キーチップが動的生成 | キーチップ由来（値判明 `45E546D2…`） | 静的かつ既知（キーチップ非依存） |
...
| 結論 | 詰み（未解決） | SSD 入手だけが条件 | 取り出しも動作も成立 |
...
| `WorkerFunc_AlphaDvdInstall` | FUN_004150f0 | DVD 読み取りと展開の本体 |
...
| `LoadBootIDHeader` | 0x418f00 | Boot ID ヘッダ読み込みと keychip 復号 |
