...
**真のブロッカー = ALL.Net / MMGP ネットワーク play-session**。boot は attract に達するが、タッチしても credit/card-auth scene が生成されない。
...
1. **entry-mode マネージャの sel 進行 gate**（`facts/gameflow.md`）: EntryMode シーン factory は `entrymode_scene_factory`(0x5ec6e0)。
   ...
   これが network/service 状態に gate されるかは未証明。cdb live トレース（0x5ec6e0 bp→戻り先 stack）が最短だが、headless は attract 手前で ~50% 停滞するため GUI/実機向き。
