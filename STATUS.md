...
genuine ブート完走まで到達（allnet.c の LFS エミュ含め commit 済）。**GAME SERVER(idx3) genuine 化は
実装・コミット済**（commit d0678f5, 2026-07-18）＝実走確証待ち。**現ブロッカーは card 受理**（card_read_sm
UID whitelist、下記フロンティア #1）＝これが元来目的かつ次の焦点。

### GAME SERVER(idx3) genuine 化 — 実装済（commit d0678f5）・実走確証待ち

**実体（`facts/devices.md`「GAME SERVER(idx3) readiness の実体」が正・詳細）**:
GAME SERVER readiness は TCP でなく **sphingo spPing の UDP 8B エコー**。ゲームは NetCli の対象(ms_url 解決 IP:ms_port)へ
...
**実装（`src/host/allnet.c`）**:
1. ✅ attend serializer で `ms_url=g_lan_ip`/`ms_port=30010`(`MS_PORT_STR`) を供給。
2. ✅ `ms_ping_listen` スレッド: `SOCK_DGRAM` で `:30010` bind、`recvfrom` した 8B を送信元(ゲームの :23456)へ `sendto` エコー。`msping.server.up`/`msping.echo` を JSONL 出力。
3. ⏳ **実走確証（残）**: 実ゲーム走行で `msping.echo` 発火＋readiness byte(`NetCli+0x11b8`)=1＋CHECKING CONNECTION の
   GAME SERVER=OK を実測。要 build+run（この checkout には build/Ghidra 無し＝別環境で実施）。gamehook に
   readiness byte READ-ONLY フック(旧 `d_gsvr_ready` 相当)を設置し確認。

**確証時に確認する未確定点**: (a) ms_url がホスト名→IP を alDns resolver(`FUN_0070de50`) 経由にせず生 IP で解決するか。
(b) 返信 8B の厳密照合（seq一致/送信元照合/長さ。`FUN_009fbcc0`/recvfrom `FUN_009ff120`）。
(c) 応答が boot 表示窓(~15s)に間に合うか。
...
- **CHECKING CONNECTION 実測（SATELLITE, LFS TCP responder 有効）= AUTH/UPLOAD/LOCAL GAME SERVER が OK**。
  UPLOAD は **PowerOn 応答を実時刻化**して genuine 解決（詳細下記＋`facts/devices.md`）。GAME SERVER(idx3) は
  **sphingo UDP 8B エコー responder を実装済**（commit d0678f5, allnet.c `ms_ping_listen`）＝実走確証待ち。
...
3. **CHECKING CONNECTION の GAME SERVER(idx3)**（実装済 commit d0678f5・実走確証待ち／上段ブロックが最新・`facts/devices.md`）:
   readiness `amlib_gameserver_ready_check`(0x72df40)＝NetCli の `NetCli+0x11b8`。**RE 確定＝TCP でなく sphingo UDP 8B ping エコー**。
   attend で `ms_url=g_lan_ip`/`ms_port=30010` 供給＋`ms_ping_listen` UDP エコー responder（8B をゲームの :23456 へ返す）を実装済。次は実走で OK 実測。
