...
3. **CHECKING CONNECTION の GAME SERVER(idx3) を OK 化**（残 1／`facts/devices.md`）: readiness `amlib_gameserver_ready_check`(0x72df40)＝`ncli::NetCli`(deviceMgr+0x7c) の
   接続バイト `netcli+0x11b4`。attend 応答の `ms_url`/`ms_port`(現状=0) が指す matching サーバへ NetCli が TCP 接続確立して初めて 1。
   ∴ 要 = valid ms_url/ms_port 供給＋接続先 loopback responder＋advertise/status 交換の genuine エミュ（**LFS 同等の新規サブシステム**）。
