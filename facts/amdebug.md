...
sink 動作の裏付け [L]: A 系統(`[lv*]` region/DNS 連鎖)・B 系統(`[raw]` amBackup* 等 retail 破棄ログ)とも窓で観測済み。
注: 起動ヘッダ（STARTUP=/GAME TITLE=）はフック確立より前に発火するため捕捉対象外（以降の無条件ログは確実に捕捉）。
...
結論: amDebug を経由する系統（A/B/C + Event ログ）は捕捉済み or 重複。SEH クラッシュ
バックトレース（系統D）は emitFrame 0x456900 POST hook で組立バッファを読取り接続済（Y: 不在でも動作）。
file ログ（last_pras/last_shime）は別内容だが未接続。

native `dbglog.c` は host 注入時に常駐（reload 非依存）。
...
| 0x55C500 | amDebugInit | startup 初期化。dbglog.c の gate 再開放 hook 先（既定値/ゲート式は上記「既定値の実体」参照）|
...
|---|---|---|---|
| 0x1696F38 | S32 | amDebug_logLevel | DebugLogLevel（既定値/ゲート式は上記「既定値の実体」参照）|
| 0x1696F3C | U32 | amDebug_logMask | DebugLogMask（同上）|
