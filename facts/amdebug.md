...
`src/host/dbglog.c`（native）が両系統を窓/JSONL へ可視化する（計装ポイントは下表。旧 Frida は `boot/amdebug/diag.js`）。
...
0x55CFD0 が出力）を含む全メッセージが捨てられる。**復元法**: `0x55C7BC`（_vsnprintf 直後）に hook を置き
`esp+0x10` の整形済みバッファを読む（関数境界外だが固定バイナリで安定。`src/host/dbglog.c` が実装）。

---

## 計装ポイント（native `src/host/dbglog.c` が実装。番地は旧 Frida `diag.js` と共通） [F]
...
| hook | 0x55C7BC | amDebugOut_format 内 _vsnprintf 直後: [esp+0x10] を `AMDBG [raw]` で送出 |

**旧 Frida 計装で捕捉した実例（native dbglog.c も同経路）**:
