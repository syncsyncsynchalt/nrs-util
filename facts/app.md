...
- self-shutdown 無力化（`src/logic/patches.c`）: 下記「ルートシーン self-shutdown」参照。
- 終了/exit ログは `src/host/exitlog.c`。

## present / 描画スレッド [S/L]

固定機能 OpenGL + 専用レンダースレッド分離レンダリング。観測対象 VA（名は doc-local、Ghidra DB 未登録）:
| static_VA | 名称 | 内容 |
|---|---|---|
...
**根因確定 = ハイブリッド GPU クロスアダプタ読戻しストール**（vsync/DWM/窓は無罪＝実証済）。毎フレ `glReadPixels`/`glCopyTexSubImage2D`（自動露出等の FB 読戻し, `FUN_0060f480` 16x16 ほか）が dGPU 描画時 PCIe 越しで≈30ms→CPU-GPU 直列化→30fps・二重 present でロード画面点滅。**恒久対応 = nrs.exe を iGPU 固定**: `HKCU\Software\Microsoft\DirectX\UserGpuPreferences` に `<exe path>=GpuPreference=1;`。
計装（撤去可）: `present.stats`/`present.swapblock`/`gl.info`(capture.c、上表 VA を観測)。ペーシング計測 `pace.main` は gamehook.c が **present drive 0x6C3930**(`d_present_drive`) を hook して frame/present/scene の avg/max us を送出（上表 0x89CB80 とは別関数）。env: `NRSEDGE_SWAP_INTERVAL`/`NRSEDGE_WINDOWED`。
...
| 0x6C3F20 | `je(0x74)→jmp(0xEB)`（byte が 0x74 のときだけ）に patch＝self-shutdown 無力化（`src/logic/patches.c`）|
