...
### amGfetcher get_status は result==success 必須 [L]

amGfetcher の全応答は `FUN_00975320`(0x975320, response dispatch) 入口で **`FUN_00975140`(0x975140, result field checker)** を必ず通る。checker: `result`=="success"→0、`invalid_parameter`/`invalid_request`/`error`→各負値、**それ以外（`result=0`）→ fall-through で -5**。∴ get_status 応答は `result=success` 必須（欠くと -5→txn 失敗→無限再接続）。
...
- settle: `pause→set_auth_params→resume→isrelease→get_status` 各1回, errors=0。
...
現行 native は serve-it: `src/host/keychip_server.c` が 40113 を bind し本物の get_status 交換で SM を自然前進させ、`src/logic/patches.c` は JL2JMP 3 個（0x97588A/0x97595F/0x975A1F）のみ。
