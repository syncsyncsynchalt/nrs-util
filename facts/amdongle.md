...
amDongleBusy(0x975E00) は amInstall SM を 40102 正式応答で完走させ busy=0 にする（下記§amInstall）。
...
- **応答 status の steer（keychip_proto.c, status→数値表は各 processor の strcmp）**:
  ...
  - query_appdata_status/check_appdata → **`error`**(-1)（FUN_00977050: available4/restored5/unknown0/error-1/needed3/checking1/formatting2）。
    -1 が 3 consumer の唯一の整合解: ①case3 query(ctx+8=0xe)は status∈{0,1,2,3,-1}で gameid 不要に成功（4/5 は gameid 必須）
    ②case4 は -1 で format 回避・state7-done（3/4/5 は format/削除誘発）③appdata task(ctx+8=0xf, FUN_00977230)は
    param∉{0,1,2}で terminate（0/1/2 は check_appdata⇄query 無限ループ）。
