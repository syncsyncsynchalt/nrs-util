# mxnetwork（amNet + ALL.Net connection）— FACTS（co-located）

mxnetwork.exe（ALL.Net / NIC 設定）相当。amNet クエリ＋ ALL.Net 接続段(state6)を扱う。
索引 `_index.md` / 横断知見 `workflow.md`。confidence: [S]=静的解析 [L]=ライブ実走 [I]=推論 [F]=旧 Frida 計装(履歴)

---

## ALL.Net 接続段（旧 patch は撤去済＝genuine ALL.Net 化, host `allnet.c`/`keychip_server.c`）

下記 2 patch は現行 `src/logic/patches.c` に不在（genuine ALL.Net emulation で state4/6 を自然通過）。アドレスは分析参照として残す。

| static_VA | 内容 |
|---|---|
| 0x72DCE0 | **（撤去済）** `FUN_0072dce0`(ALL.Net server status, amlib_device_status_getter) → 2(ready) 固定。SYSTEM STARTUP state4(network) + state6(allnet) 両立（詳細 `mxsegaboot.md`） |
| 0x6FF1B3 | **（撤去済）** `FUN_006ff140` LAN フラグ **b50c** init 0→1（Error 8005 WAN 抑止, state4/6 の gate: SM の `&0x5f3` が含むのは bit 0x100=b50c のみ。兄弟 b50a=bit0x800/b50b=bit0x1000 は mask 外で無効のため patch 不要） |

将来ネットワーク対応（ALL.Net 4 サービス: Authentication PowerOn / DownloadOrder / Billing / AiMeDB と BBS
タイトルサーバ）は **network_role=serve**（実サーバ応答へ移す層）で、仕様は **正本＝RingEdge 1 純正 `system\mxnetwork.exe`/`mxmaster.exe` 等**、TeknoParrot は補助（`ref.md` 階層）として要再 RE。billing は `ambilling.md`、storage は `mxstorage.md` に分離済。
...
### PCPA wire format for amNet — fields `&`-separated [F]

パーサ `FUN_0098bb30`(static 0x98bb30): `&`→新 key=value / `=`→k/v 区切り / `\r`|`\n`→**パース終了**。
k/v テーブル= stream+0x100(keyword)/+0x140(command)/+0x180(件数)、`pcpaGetCommand`(FUN_0098aae0→FUN_0098b610) が引く。
1 行 `&` 区切り、末尾 `\r\n>` を付ける（`\r\n` を field 区切りに使うと最初の `\r` で打切→`result` 欠落→-1→SM loop→Error 8006）。
- `response=<req>` 先頭キー必須（無→抽出器 -6）。`result=0` 必須（atoi=0 が成功パス、無→-1）。
- served: `src/host/keychip_server.c` 40104。native 成功=ret0/dhcp_status=3/nic_ready=1。

### 応答抽出と nic プロパティ連鎖 [S]

- **`pcpa_response_extractor`** (static 0x9814E0 / RVA 0x5814E0): `response`/`result` を見て ctx+0x10
  (req_type_idx) で分岐。dhcp=case1/2、nic=**case9/10 → `FUN_009807f0` に委譲**、mac=case0xd/0xe。
  `result` 欠落 or 非0で switch default → **-1**。
- **`FUN_009807f0`** (static 0x9807f0, nic プロパティ取得連鎖 SM): `query_nic_status` の
...
bind は現状エミュでは介入していない（socket フック無し）。native でどう成立するかは未再 RE。
介入が必要になった場合のみ再調査する。
