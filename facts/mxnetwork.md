...
### PCPA wire format for amNet — fields are `&`-separated [F]
...
amNet 応答を `\r\n` 区切りにすると最初の `\r` でパースが止まり `response` ペアのみ登録され、抽出器が
`result` を見つけられず -1 → SM ループ → Error 8006 になる。`src/host/keychip_server.c`(amNet ポート 40104) は
`&` 区切りで応答し、native 成功（ret=0, dhcp_status=3, nic_ready=1）する。
