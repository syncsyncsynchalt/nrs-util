...
## ALL.Net 接続段（genuine ALL.Net 化, host `allnet.c`/`keychip_server.c`）

state4/6 は genuine ALL.Net emulation で自然通過。関連 VA（分析参照。patches.c に該当 patch は無し）:

| static_VA | 分析参照 |
|---|---|
| 0x72DCE0 | `FUN_0072dce0`(ALL.Net server status, amlib_device_status_getter)。SYSTEM STARTUP state4(network) + state6(allnet) 両立（詳細 `mxsegaboot.md`） |
| 0x6FF1B3 | `FUN_006ff140` LAN フラグ **b50c**（Error 8005 WAN 抑止, state4/6 の gate: SM の `&0x5f3` が含むのは bit 0x100=b50c のみ。兄弟 b50a=bit0x800/b50b=bit0x1000 は mask 外） |
...
- served: `keychip_proto.c`（bind は `keychip_server.c` 40104）。native 成功=ret0/dhcp_status=3/nic_ready=1。
