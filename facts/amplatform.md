...
Error 0901/AAL latch の真因もこの `FUN_0045a6f0`。両関数の静的パッチは撤去しエミュ供給へ移行済み: PlatformId は columba DMI、OsVersion は仮想 `SystemVersion.txt`（下記）。`args[1]`(bufLen) はスタックゴミなので無視。
...
columba は host エミュ済（`src/logic/driver/mxdevices.c` build_dmi が index 2="AAL" を返す）＝静的パッチ不要で撤去済み。再発時のみ復活。

**OsVersion(0x981D60) も撤去済み = 仮想ファイル化。** gate `FUN_0045a6f0` は OsVersion の **戻り値が 0 か**だけを見る（値 local_10 は直後の memory size 取得で上書き＝捨てる。caller は gate ただ一つ）。
