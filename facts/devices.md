...
**メーカー特定＝不能（2026-07-01, 全オラクル sweep 済・二度解かない）**: 物理リーダーの vendor/型番は**どの成果物からも判定できない**。根拠:
① nrs.exe = vendor/model/version 文字列ゼロ（照会コマンドも送らない, 上記）。② 純正 segadriver(`common\segadriver\`) に card 専用 .sys 無し＝COM2 は汎用シリアル扱い。
③ micetools の唯一の card シリアル機器 `ser_gacchu_guts_card.c` は**別プロトコル**（length-prefix + `0x03` tail + ASCII "C2"/"Z8"、別ゲーム「Gacchu Guts」COM1、`games.c:193`）＝Border Break 無関係。
④ TP `GameProfiles\BorderBreakScramble.xml` に card emulation 項目ゼロ（未エミュ）。→ 唯一言えるのは**プロトコル形状からの推測**のみ:
multi-slot(最大7) addressing + `card_eject`/`card slot ok` = **払い出し/取込機構つき card dispenser**、種別バイト→容量(1K/2K/4K−64Bヘッダ)= 接触/非接触メモリカード媒体。

**Web sweep（2026-07-01, ref 権威④相当の外部知見）で候補を絞込み**:
- RingEdge の **COM2=カードリーダー**は外部でも一致（sega コミュニティ）。SEGA カードR/Wは**2世代**存在:
  ① **Sanwa CRP-1231BR-10 / CR-S31R-10 系**＝**磁気**(3トラック×69B=207B)・**8N1**・**STX/length/ETX＋checksum のフレーム化 ASCII コマンド**（WMMT/Initial D3 NAOMI世代, YACardEmu が emulate, micetools `gacchu_guts_card` の `[len][cmd][0x03]` もこれ）。→ **本機とプロトコル全不一致＝除外確定**（本機は IC・8E1・裸バイト・checksum無）。
  ② **`CR-2PR-10H-SG`**＝**RingEdge/RingWide 世代**の IC(Mifare)カードR/W（Initial D Arcade Stage 4-8 が全て採用。旧磁気機と「別カード・別機能」と明言）。COM2 接続。
- → **Border Break(RingEdge COM2 IC Card R/W) の最有力候補＝`CR-2PR-10H-SG` クラスの Sanwa 系 IC カードR/W**（型番 `CR-…-SG`＝Sanwa 命名＋Sega 向け suffix, SEGA部品番号運用）。**確度: 高いが未確定**（本機固有型番の一次確認と、CR-2PR-10H-SG の "Sanwa" 明記は未取得。protocol byte 一致の裏取りは公開資料に無し）。旧磁気 Sanwa の除外のみ確定事実。
...
- **Phase B2 配線済（2026-07-01, データ授受は live 未確証）**: ローダー「カード」タブ + 制御ファイル IPC + card.c R/W 配線を実装。
  - **IPC**: loader が `nrsedge.card.json`（present/image/type/gen）を書き、logic の `card_control_poll`(on_jvs_tick)が
    last-write 変化時に `host->orig(CreateFileW/ReadFile)`（フック迂回）で読み image を `st->card.image[0x1008]` へ反映。
    **insert/eject/UID-load は live 検証済**（`card.insert` uid=deadbeef/type=ff、`card.eject` 発火）。card.bin 永続化=eject 時 dirty 保存。
  - **card.c build_response 暫定**: 0x4D present=`[0B][1A][type][UID 4B BE][pad]`、0x2D=4B addr+4B len BE→read_cursor、
    0x0D=`2B`+image[cursor..+128]（card.bin=純ワイヤ image・swap 無し）、0xED/0x8D=`8B`+容量2B、0xAD=4B addr→write_addr。
  - **残（live 確証要）**: 8B UID record の正確な byte 順 と 0xAD 後続 write データのフレーミング。`coin/touch→card-login` で
    0x4D/0x2D/0x0D/0xAD を駆動し、カードタブの hex + card.* ログで実トラフィックと突合して確定する（確定後ここを done 昇格）。

- **★★ ゲーム側カード認識 = 解決（2026-07-01 live 確証）**: 2 つの根因を特定・修正し、SEARCH→select→read flow が完走（クラッシュ無し）:
  1. **standalone にカード挿入の out-of-band 検出信号が無い** → `api.c card_force_present`: present 時に `device_status+0x5628`(presence/read-trigger)=1 を供給
     → `FUN_004f6a30` present 化＋read シーケンサ `FUN_004f30a0` がキックされ **SEARCH(0x4D)発火**（attract でも背景検出が走る）。
     ※ flags/state2 の直接 poke 版はクラッシュした（生 SM 撹乱）。**presence フィールドのみ in-band 供給する版は安定**。
  2. **SEARCH 応答のフレーム長バグ**: Phase B 改変で 0x4D present 応答を 8B に縮めていた（payload 7B）。frame 則は **0x0B→9B**（payload 8B,
     `card_get_uid_record` 0x885260 が payload を `DAT_01265722..29` に取込）。1B 不足で re-sync 崩壊→read SM が -0x61(-97)→クラッシュ。
     **9B に修正**（`0B 1A type UID[4] 00 00`）→ ゲーム受理→`0x2D`(select by UID)→`0x8B` ACK→`0x0D`(read block) へ前進。live 確証: UID `b5ce5991` 認識。
  - **シリアル read/write = 完動（2026-07-01 live 確証）**: `0x0D` read block は **3 バイト `0d HH LL`**（static の単バイト推測は誤り。1B 扱いだと続く `00 00` が
    0x2B+128B 応答を card_build_response の tx リセットで消去し「データ読み込み中…」停止）。`0x2D` は **select-by-UID**（read addr ではない）。
    修正後: SEARCH→select(0x2D)→read(0x0D×N, 128B/回)→commit(0xAD)→halt(0x38) が完走、「読み込み中」突破。read config は `FUN_00674800`
    が設定（read 中 `DAT_016a55ba`=bypass=1 / maxlen=0xFC0 / whitelist count=0）＝**whitelist は実は照合されない**（起動時診断の 0 は read 前のため）。
  - **★★★ 「このカードは使用できません」の真因 = ALL.Net カード情報ネットワーク（カードデータ形式ではない）**:
    card-auth scene(0x5e6200) は読んだ UID で **`RequestSendAdapter<NetDataCardinfoRequest>`(FUN_007203e0) をサーバ送信し
    `ResponseRecvAdapter<NetDataCardinfoResponse>`(FUN_007205b0) を受信**してプレイヤープロフィールを得る。**プロフィールはサーバ側**で、
    カード(4032B)は ID に過ぎない。standalone は応答が得られず「使用できません」。net.connect 先 = `127.0.0.1:40113`。
    シリアライザ = `NetDataCardinfoRequest/Response::class2Binary/Binary2Class`(0x90f2b0/0x90f3e0/0x913460/0x913590, 非パック RE 可)。
    → **カード使用化 = ALL.Net カード情報応答のエミュレート**（PCP/network system 層。netauth.force_ready 流に応答詐称も理論上可）。
    カードデータ形式の forge では解決しない（方向転換確定）。
  - **「使用できません」チェックの精密な所在（2026-07-01・逆算用）**: 3 段すべて NUPL ネットワークタスク依存:
    1. `FUN_00717da0`: task list(DAT_016db564) を uid=`0x4c50554e`("NUPL"=ALL.Net session task)で探し、ctx=`*(node+0x10)` の **+0xd8（応答ステータス）** を返す。
       `FUN_007205b0` はこれ==0（応答受信済）のときだけ処理。standalone は NUPL 応答が来ず ≠0 → カード情報パース不能。
    ...
    → 注入点 = **NUPL task ctx**（+0xd8=0 にし、応答 binary 域に status=1＋profile の `NetDataCardinfoResponse` を置く）。
       「妥当データ」はカード上ではなく**サーバ応答**なので、ALL.Net カード情報応答フォーマットの実装が前提（PCP/network 層の本丸）。
  - 以下は解析の核（参照用に温存）:
  - **重大事実: card-select 画面でもゲームは serial SEARCH(0x4D) を一切撃たない**（全セッションで 0x4d=0件。撃つのは handshake の f7/68/48/b8 のみ）。
    ＝認識は**シリアル経由でなく cardrw object SM の memory state 経由**。present 詐称＝serial emu を ready にするだけでは届かない。
  - **scene の present ゲート（全 case 共通）**: `obj=node[4]`（class 0x21,0x21 ノード、`cardrw_device_status_ptr` 0x4f3e30）。
    `device_status = obj+4`。判定 = **`*(int*)(device_status+0x5628) != 0`（presence/read-seq state）** かつ `device_status flags(+0)&0x10(error)==0`、または `FUN_004f2d20()`。
  - **cardrw_object_update_sm(0x4f2ad0) 検出鎖**: case0 state2(=`device_status+8`) 0→1 は `flags&0xc0`; case1 `advance_to_present`(0x4f6810, state2 1→2＋flags|0x1000)
    呼出は `(flags&0x400)==0 && bit7(0x80)clear`; case2 は presence query `FUN_004f6a30`(=`device_status+0x5628!=0`)が真でないと hold せず state2→0 へ revert。
  - **read シーケンサ `FUN_004f30a0`** は `device_status+0x5628` を state とする（case0 が zero にするのは隣の `+0x562c`）。`advance_to_present` が呼ぶ `FUN_004f3c80` は buffer memset のみで read 要求ではない。
  - **試行と教訓**: SM の flags/state2 を logic から poke して state2 0→1→2 まで進めたが presence(+0x5628)=0 で hold せず振動 → **ゲームが access violation でクラッシュ**（host.dll+C8D2, 0xC0000005）。
    **生きた SM の途中状態を外から書く provision は危険**（eeprom/dipsw force_ready が安全なのは init 前の空 ctx を一回埋めるだけだから）。撤去済。
  - **次の照準（未試・要 card-select 画面で慎重に）**: `device_status+0x5628 = 1` を供給（real HW のカード挿入検出信号相当）→ `FUN_004f6a30` present 化＋`FUN_004f30a0` を read 列へ。
    観測用に `card.sm` 診断（`api.c card_sm_diag`: devf/sub/cls/req + dsflags/state2/presence）を温存。**card-select 画面で「カード使用」を見ながら**反復確証する（attract では scene 非活性ゆえ最終確証不可）。
...
> 正は `0x4f...`。`0x8f63xx` は無関係な JcvCard デシリアライザ（`FUN_008f33e0`）。Ghidra DB 訂正済み。
