...
**GAME SERVER(idx3) readiness = sphingo spPing UDP 8B エコー（実装済 commit d0678f5・実走確証待ち）**:
> 実装＝`src/host/allnet.c`: attend serializer で `ms_url=g_lan_ip`/`ms_port=30010` を供給し、`ms_ping_listen` スレッドが `:30010`(UDP) で 8B を受け送信元(ゲームの :23456)へエコー返送。responder 契約（下記）を満たす。**実ゲーム走行での readiness=1 確認は未取得**（この checkout に build/Ghidra 無し）。
