...
- **DeviceIoControl**: PING `0x9c406000`→`0x01000001` / **GET_SECTOR_SIZE `0x9c406004`→4(RINGEDGE2, micetools 由来＝純正で要確認※)** /
  ...
  ※SRAM record は全て 512B/512境界ゆえ 512 でも動く。値 4 は micetools 由来で採用（純正 mxsram.sys は GET_SECTOR_SIZE を memcard0 へ転送＝自前で答えない＝純正からは確定できない。下記「実 RingEdge ドライバ」節）。前回「512 だと弾かれる」は誤り。
...
これで R/W は通るが、上記 STATIC seed（region=01/serial/CRC）は**別途必要**だった → **実装済**（`src/logic/driver/mxdevices.c` `eeprom_seed_static`: AM_SYSDATAwH_STATIC を region=01/serial/amiCrc32R 正CRC で REG@0x000 へ、blank/無効時のみ）。これが純正供給経路（基板 EEPROM area0=STATIC → `amBackupRead` → `region_game_pcb` 0x16014c4）の再現。詳細 `mxkeychip.md` region 表。`amEepromInit` 不要（force_ready が H_MXSMBUS で provisioning 済）。要ライブ検証: STATIC `is broken=0` と Error 0903 消失。
...
- **sector size: RINGEDGE2=4**（micetools `MICE_PLATFORM_RINGEDGE2 ? 4 : 512`）。**出所は micetools のみ**＝純正
  ...
  geometry の BytesPerSector=512 とは別物。→ エミュは GET_SECTOR_SIZE に **4** を返す。**注意（前回訂正）**: SRAM 系 record は
  AM_SYSDATAwH_{BACKUP,HM_PEAK,TIMEZONE,ERROR_LOG} とも**明示パディングで正確に 512B/512境界**なので 512 でも
  アラインメント検査は通る＝洪水の原因は sector size ではなく**データ面の欠落のみ**。4 は厳密に緩いので採用（純正で要確認）。
...
エミュ: `mxdevices.c` の `build_dmi` が DMI 192B を組む（micetools `dmi_build_default()` の RINGEDGE2 分岐を参考に実装。DMI 値の正は純正 columba.sys/実機 DMI）。
