...
**EEPROM は名前 CreateFile でなく SetupDi で開く**(`amEepromCreateDeviceFile` 0x984910)。standalone では
`api.c eeprom_force_ready()` が eeprom ctx を poke して provisioning（`amEepromInit` 0x985160 を経ずに）: ctx base
`0xccf4e0`（`+0x00`=initFlag=1 / `+0x04,+0x08`=6 / `+0x0c`=mutex(CreateMutexA) / `+0x10`=device handle=**H_MXSMBUS** /
`+0x14`=i2c 0x57）。handle を H_MXSMBUS にすると write/read fn の `DeviceIoControl(0x9c40200c, i2c@0x57)` が mxsmbus
エミュ(AT24C64AN→eeprom.bin)へ流れる。on_jvs_tick は storage init 後に回るため initFlag==0 を安全に上書きする。
STATIC seed（region=01/serial/CRC）は `src/logic/driver/mxdevices.c` `eeprom_seed_static`（AM_SYSDATAwH_STATIC を
region=01/serial/amiCrc32R 正CRC で REG@0x000 へ、blank/無効時のみ）＝純正供給経路（基板 EEPROM area0=STATIC →
`amBackupRead` → `region_game_pcb` 0x16014c4）の再現。詳細 `mxkeychip.md` region 表。

`ENABLE_EEPROM=false` で eeprom を発見不可にし degraded パス（records -3）へ戻せる。
  ...
  → amBackupRead(0x985a00) / amBackupWrite(0x985940)                  記録プリミティブ
      → amBackupReadRecord(0x9858D0)                                  area記述子→デバイス read
          ...
          → amBackupReadDevice(0x985770) / amBackupWriteDevice(0x985820)  記述子+0xc(read)/+0x10(write) fn ptr 呼出
              → area→fn/バッキング対応は下記「記述子テーブル分岐」表を正とする
...
### native エミュ実装（データ面）[S]
host が `SetFilePointer` をフック→`on_set_file_pointer`→`mxdev_seek`。`mxdev_read`/`mxdev_write` が
memory-map した flat バッファ(SRAM=`nvram.bin` 0x200000B / EEPROM=`eeprom.bin`)へ memcpy（永続化・map 方式は上記 mxsram 節）。ABI v4 で追加。
...
native が columba DMI=RINGEDGE2 を emulate し board type 3 を確定させる（`DAT_01296158`/`DAT_00ccf450` キャッシュ）。
未エミュ時は `getAreaDescriptor` null → `-21`(0xffffffeb)。`amBackupRecordRead/WriteDup error` は columba とは別因＝mxsram データ面（下記）。
