...
| 5 | CHECKING EXTEND IMAGE | `FUN_0072b3a0`(install status) / `FUN_004fda50`(is-DVD-boot, case1) | →4(done)+*ESI=0 で install skip ＋ DVD-boot→0 | `patches.c`(0x72B3A0 / 0x4FDA50, `mxstorage.md`)。⚠️0x72B3A0 を消すと画面が "EXTEND IMAGE … NG / INSTALLING … WAITING" で停止する。**BOOT_DONE(HLSM `FUN_00457FE0`) は SYSTEM STARTUP SM と別物**でこの停止を検出できない |
