...
**実測ステータス（2026-06-28, 深刻度=間欠・低頻度）**: 旧 Frida probe（描画 choke
...
効いている。残るレース（おそらく 0x14 network 早期 or cold-start 依存）は低頻度。
再現時は `error_scene_render`(0x6f2730) を host hook し caller 戻り番地を JSONL ログ化して dispatch 元を捕捉する（Frida 廃止後は host hook / cdb で代替）。
...
（ref=`DAT_0160198a`）。nic IP は query_nic_status 応答の inline 解析で ctx+0x3c へ入る（`mxnetwork.md`）。

**次の一手（具体）**: 静的は関数ポインタ越えで止まるので、**runtime で `error_scene_render`(0x6f2730) を
host hook し caller 戻り番地＋param_1(記述子)を JSONL ログ化して出所を特定**する（cdb の bp+kb でも可。B1 の教訓＝
...
実は **旧 Frida region モジュールの `setInterval(250ms)` watchdog**（毎 250ms `0x16F5AF0=0`）を native と取り違えたもの。
従って native の latch は単純に「最初の errCode が次の reset まで固着」。**handled errCode の fix は `src/logic/patches.c`
の静的パッチ**、各 setter 関数は要時 Ghidra `search_decompiled` で
再導出する（正は実装）。描画 choke point と次の一手は上の Root cause 参照。
- **0xa board-table**(`FUN_00679cb0`)= **解消済**。board index `DAT_01601953` を 2 に固定（`patches.c`/dipsw エミュ,
  0x45A18E 系）して table[2]=8 を満たす＝値供給。setter NOP ではなく入力を正す方式（`FUN_006c5470` の
  0x11/0x1e も同時解消）。詳細 `devices.md`。
...
⚠️ いずれも stable attract（既達）を壊さないこと。検証は実写（GUI のスクリーンショット）必須。
...
## [RISK] JVS 入力注入: ネイティブ JVS 経路の再駆動は禁止（旧 amjvs/input.js 初版の教訓）

**症状**: 旧 input.js 初版（Frida）は poll_state(`va(0x16B7EA0)`)を毎フレーム再アームし、amlib リーダ
...
- credit 経路の実体は **amCredit**（`DAT_01288550` ブロック、init=`amCreditInit` 0x97D320。freeplay 相当は現 `api.c` on_jvs_tick）。
...
inner_input を触らずに入力が反映される（旧 Frida probe で裏取り。再取得は Ghidra/実走）。
...
coin/credit は **billing にブロックされない**（上記訂正）。旧 direct-write 方式では native リーダ経路を
forgery が失敗扱いにして credit_handler が走らなかった＝**未実装であってブロックではない**。実装するなら amCredit
...
Safe bypass: hook HLSM onEnter to write param_1[0x18]=8 BEFORE the first-switch runs.
...
`src/logic/patches.c` の `{0x457AF0, RET8_0, 5}` は、注記が「keychipSM state4 crash helper」だったが誤りである。
実体は `delete_directory_recursive`(0x457af0) の無効化で、`xor eax,eax; ret 8` でこの関数を即 0 返しにし、ゲームによる実ディレクトリの再帰削除を阻止している。

⚠️ **スタブは `ret 8` 必須（bare ret は不可）**: 0x457af0 は **`__thiscall(this, arg1, arg2)`＝callee-clean で 8B のスタック引数を `ret 8` で掃除する規約**（Ghidra 裏取り: 関数末尾 `0x457fd1 RET 0x8` / 呼出元 `0x4579F4` の CALL 直後に `add esp,8` 無し / 再帰自己 call 0x457DE3 も同様）。スタブを bare `ret`(旧 `RET0`=`31 C0 C3`)にすると 8B を残してスタックがずれ、**発火時にスタック破壊→クラッシュ**する。正しいスタブは `RET8_0`=`31 C0 C2 08 00`（5B）。amDongleBusy(0x975E00) が `RET0` で正しいのは引数 0 だから＝**ret の即値は関数ごとに実体で確認する**。
